...
public class CartDAO {
	 ...
	 * p.PRODUCT_PRICE, po.OPTION1_TYPE, po.OPTION1_VALUE, po.OPTION2_TYPE,
	 * po.OPTION2_VALUE, po.OPTION3_TYPE, po.OPTION3_VALUE, NVL(po.PRICE, 0) AS
	 * OPTION_PRICE FROM PRODUCT_OPTION po JOIN PRODUCT p ON po.PRODUCT_NO =
	 * p.PRODUCT_NO WHERE po.OPTION_ID IN (%s) ORDER BY p.PRODUCT_NO DESC
	 * """.formatted(placeholders);
	 * 
	 * try ( Connection conn = ConnectionProvider.getConnection(); PreparedStatement
	 * pstmt = conn.prepareStatement(sql) ) { int index = 1;
	 * 
	 * for (Integer optionId : guestCart.keySet()) { pstmt.setInt(index++,
	 * optionId); }
	 * 
	 * try (ResultSet rs = pstmt.executeQuery()) { while (rs.next()) { int optionId
	 * = rs.getInt("OPTION_ID"); int productPrice = rs.getInt("PRODUCT_PRICE"); int
	 * optionPrice = rs.getInt("OPTION_PRICE");
	 * 
	 * CartItemDTO dto = new CartItemDTO();
	 * 
	 * dto.setOptionId(optionId); dto.setQuantity(guestCart.get(optionId));
	 * dto.setProductNo(rs.getInt("PRODUCT_NO"));
	 * dto.setProductName(rs.getString("PRODUCT_NAME"));
	 * dto.setProductPrice(productPrice); dto.setOptionPrice(optionPrice);
	 * dto.setUnitPrice(productPrice + optionPrice);
	 * dto.setOption1Type(rs.getString("OPTION1_TYPE"));
	 * dto.setOption1Value(rs.getString("OPTION1_VALUE"));
	 * dto.setOption2Type(rs.getString("OPTION2_TYPE"));
	 ...
	 * RuntimeException("비회원 장바구니 조회 실패", e); }
	 * 
	 * return list; }
	 */
	public List<CartItemDTO> getGuestCartItems(Map<Integer, Integer> guestCart) {
	    ...
	    String sql = """
	        SELECT
	            ...
	            (
	                ...
	                WHERE pi.OPTION_ID = po.OPTION_ID
	                ORDER BY
	                    CASE
	                        WHEN pi.IMAGE_PURPOSE = '대표' THEN 0
	                        ELSE 1
	                    END
