...
<body>
  ...
  <c:if test="${totalPages > 1}">
    <nav class="pagination" aria-label="페이지 이동">
      ...
      <c:if test="${page > 1}">
        <a href="${pageContext.request.contextPath}/admin/vendor-action-logs?page=${page - 1}">&larr;</a>
      </c:if>

      <c:forEach var="p" begin="1" end="${totalPages}">
        <a class="${p == page ? 'active' : ''}" href="${pageContext.request.contextPath}/admin/vendor-action-logs?page=${p}">${p}</a>
      </c:forEach>

      <c:if test="${page < totalPages}">
        <a href="${pageContext.request.contextPath}/admin/vendor-action-logs?page=${page + 1}">&rarr;</a>
      </c:if>
