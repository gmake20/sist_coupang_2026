	...
	<c:otherwise>
		<c:forEach var="payment" items="${paymentMethods}" varStatus="status">
			...
			<label class="wow-payment-method"> <input type="radio"
				...
				${payment.paymentDefault || status.first ? 'checked' : ''}>

				<div class="wow-payment-method-info">

					<c:choose>
						<c:when test="${payment.paymentType eq 'BANK'}">
							<strong> <c:out value="${payment.bankName}" />
							</strong>

							<span> 계좌 **** <c:out value="${payment.accountLast4}" />
							</span>

							<span> 예금주 <c:out value="${payment.accountHolder}" />
							</span>
						</c:when>
						...
						<c:when test="${payment.paymentType eq 'CARD'}">
							<strong> <c:choose>
									...
									<c:otherwise>
										<c:out value="${payment.cardCompany}" />
									</c:otherwise>
								</c:choose>
							</strong>

							<span> 카드 **** <c:out value="${payment.cardLast4}" />
							</span>
