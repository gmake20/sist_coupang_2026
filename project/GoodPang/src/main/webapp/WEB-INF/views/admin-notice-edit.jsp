...
<body>
  ...
  <form method="post" action="${pageContext.request.contextPath}/admin/notice/edit">
    ...
    <div class="field">
      ...
      <select name="noticeType">
        <option value="공지" ${notice.noticeType == '공지' ? 'selected' : ''}>공지</option>
        <option value="안내" ${notice.noticeType == '안내' ? 'selected' : ''}>안내</option>
      </select>
    </div>

    <div class="field">
      <label>제목</label>
      <input type="text" name="title" maxlength="200" required value="${notice.title}">
    </div>
