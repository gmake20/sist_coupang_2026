...
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/member_withdraw_check.css">
</head>
<body>
        ...
        <main class="withdraw-container">
            ...
            <div class="withdraw-header">
                ...
                <div class="withdraw-step">
                    ...
                    <strong>02 GoodPang 이용내역 확인</strong>
                    <span class="arrow">›</span>
            ...
            <section class="history-section">
                    ...
                    <div class="table-row membership-grid">
                        <span>
                            <c:choose>
                                ...
                                <c:otherwise>이용 중인 멤버십 없음</c:otherwise>
                            </c:choose>
                        </span>

                        <span>
                            <c:choose>
                                <c:when test="${wowActive}">${nextPaymentDate}</c:when>
                ...
                <c:if test="${wowActive}">
                    <p class="warning-text">와우 멤버십을 이용 중인 경우 멤버십 해지 후 회원 탈퇴가 가능합니다.</p>
                </c:if>
            </section>
            ...
            <section class="history-section">
                ...
                <div class="history-table">
                    <div class="money-row">
                        <span>보유 GoodPay 머니</span>
                        <strong><fmt:formatNumber value="${goodPayBalance}" pattern="#,###" />원</strong>
                    </div>
                </div>

                <c:if test="${goodPayBalance > 0}">
                    <p class="warning-text">보유하고 있는 GoodPay 머니가 있습니다. 탈퇴 전 잔액을 확인해주세요.</p>
                </c:if>
            </section>
            ...
            <section class="history-section">
                ...
                <div class="history-table">
                    <div class="money-row">
                        <span>환불 처리 예정 금액</span>
                        <strong><fmt:formatNumber value="${refundAmount}" pattern="#,###" />원</strong>
                    </div>
                </div>
            </section>
            ...
            <section class="history-section">
                <h3>소멸 예정 금액</h3>
                <p class="section-description">무료로 지급된 쿠폰 및 혜택은 회원 탈퇴 시 즉시 소멸되며 복구되지 않습니다.</p>

                <div class="history-table">
                    <div class="money-row">
                        <span>GoodPang 쿠폰</span>
                        <strong>${couponCount}개</strong>
                    </div>
                </div>
            </section>

            <form id="withdrawCheckForm" action="${pageContext.request.contextPath}/member/withdraw/complete" method="post">

                <label class="final-agree">
                    <input type="checkbox" id="finalAgree">
                    <span>위 이용내역과 소멸 예정 혜택을 확인하였으며 회원 탈퇴에 동의합니다.</span>
                </label>
                ...
                <div class="withdraw-buttons">
                    <a href="${pageContext.request.contextPath}/member/withdraw" class="btn-prev">이전</a>
                    <button type="submit" class="btn-withdraw" ${withdrawBlocked ? 'disabled' : ''}>탈퇴하기</button>
                </div>
