...
<main class="main">
	...
	<div class="page-head">
		...
		<div class="page-actions">
			...
			<a class="btn btn-primary"
				href="${pageContext.request.contextPath}/vendor/product_write.htm">상품
				등록</a>
		</div>
	...
	<div class="view-tabs">
		<a class="view-tab ${not hiddenView ? 'active' : ''}"
			href="${pageContext.request.contextPath}/vendor/product.htm">노출 중</a> <a
			class="view-tab ${hiddenView ? 'active' : ''}"
			href="${pageContext.request.contextPath}/vendor/product.htm?view=hidden">숨김</a>
	</div>
	...
	<section class="stat-row">
		...
		<article class="stat-card">
			<div class="stat-top">
				<span class="stat-label">전체 상품</span> <span
					class="stat-icon stat-icon-blue"><svg class="icon">
						<use href="#ic-box" /></svg></span>
			</div>
			<div class="stat-value">${totalCount}
				<small>개</small>
			</div>
			<a href="#" class="stat-link">전체보기 <svg class="icon">
					<use href="#ic-chevron-down" /></svg></a>
		</article>

		<article class="stat-card">
			<div class="stat-top">
				<span class="stat-label">판매 중</span> <span
					class="stat-icon stat-icon-green"><svg class="icon">
						<use href="#ic-bag" /></svg></span>
			</div>
			<div class="stat-value">${saleCount}
				<small>개</small>
			</div>
			<span class="stat-rate rate-green"> <fmt:formatNumber
					value="${totalCount > 0 ? (saleCount * 100.0 / totalCount) : 0}"
					pattern="0.0" />%
			</span>
		</article>
		...
		<article class="stat-card">
			<div class="stat-top">
				<span class="stat-label">품절</span> <span
					class="stat-icon stat-icon-orange"><svg class="icon">
						<use href="#ic-ban" /></svg></span>
			</div>
			<div class="stat-value">${soldOutCount}
				<small>개</small>
			</div>
			<span class="stat-rate rate-orange"> <fmt:formatNumber
					value="${totalCount > 0 ? (soldOutCount * 100.0 / totalCount) : 0}"
					pattern="0.0" />%
			</span>
		</article>
		...
		<article class="stat-card">
			<div class="stat-top">
				<span class="stat-label">판매 중지</span> <span
					class="stat-icon stat-icon-red"><svg class="icon">
						<use href="#ic-pause-circle" /></svg></span>
			</div>
			<div class="stat-value">${stoppedCount}
				<small>개</small>
			</div>
			<span class="stat-rate rate-red"> <fmt:formatNumber
					value="${totalCount > 0 ? (stoppedCount * 100.0 / totalCount) : 0}"
					pattern="0.0" />%
			</span>
		</article>
		...
		<article class="stat-card">
			<div class="stat-top">
				<span class="stat-label">승인 대기</span> <span
					class="stat-icon stat-icon-blue"><svg class="icon">
						<use href="#ic-clock" /></svg></span>
			</div>
			<div class="stat-value">${pendingCount}
				<small>개</small>
			</div>
			<span class="stat-rate rate-blue"> <fmt:formatNumber
					value="${totalCount > 0 ? (pendingCount * 100.0 / totalCount) : 0}"
					pattern="0.0" />%
			</span>
		</article>
	...
	<section class="panel filter-panel">
		...
		<div class="filter-row">
			...
			<div class="filter-field">
				<label>상품명</label> <input class="input" type="text"
					placeholder="상품명을 입력해주세요.">
			</div>

			<div class="filter-field">
				<label>카테고리</label> <select class="input select">
			...
			<div class="filter-field">
				<label>판매 상태</label> <select class="input select">
					<option>전체</option>
					<option>판매 중</option>
					<option>품절</option>
					<option>판매 중지</option>
			...
			<div class="filter-field">
				<label>상품 유형</label> <select class="input select">
					<option>전체</option>
					<option>일반상품</option>
					<option>로켓배송</option>
					<option>정기배송</option>
			...
			<div class="filter-field">
				<label>브랜드</label> <select class="input select">
					<option>전체</option>
					<option>코멧</option>
					<option>로켓프레시</option>
					<option>탐사</option>
	...
	<div class="result-toolbar">
		...
		<div class="result-controls">
			...
			</select> <select class="input select select-sm">
	...
	<section class="panel table-panel">
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="product" items="${productList}">
									...
									<div class="product-cell">
										<c:choose>
											<c:when test="${not empty product.thumbnailUrl}">
												<span class="thumb"> <img
													src="${img:url(product.thumbnailUrl)}"
										...
										<div class="product-text">
											<p class="product-name">
												<a
													href="${pageContext.request.contextPath}/vendor/product_detail.htm?productNo=${product.productNo}"
													style="color: inherit; text-decoration: none;">${product.productName}</a>
											</p>
											<p class="product-sku">상품번호 ${product.productNo}</p>
											<p class="product-cat">${product.mainCategoryName}&gt;
												${product.midCategoryName} &gt; ${product.subCategoryName}</p>
										</div>
									</div>
								</td>
								<td class="col-price"><c:choose>
                          ...
                          ₩<fmt:formatNumber value="${product.minPrice}"
												pattern="#,##0" /> ~
                          ₩<fmt:formatNumber value="${product.maxPrice}"
												pattern="#,##0" />
										</c:otherwise>
									</c:choose></td>
								...
								<td class="col-date"><fmt:formatDate
										value="${product.createdDate}" pattern="yyyy-MM-dd" /><br>
									<span class="time"><fmt:formatDate
											value="${product.createdDate}" pattern="HH:mm" /></span></td>
								<td class="col-date"><fmt:formatDate
										value="${product.updatedDate}" pattern="yyyy-MM-dd" /><br>
									<span class="time"><fmt:formatDate
											value="${product.updatedDate}" pattern="HH:mm" /></span></td>
								<td class="col-manage"><a class="btn btn-outline btn-sm"
									href="${pageContext.request.contextPath}/vendor/product_detail.htm?productNo=${product.productNo}">상세보기</a>
									<c:choose>
										<c:when test="${hiddenView}">
											<form method="post"
												action="${pageContext.request.contextPath}/vendor/product_visibility.htm"
		...
		<c:if test="${totalPages > 1}">
			<nav class="pagination" aria-label="페이지 이동">
				...
				<c:if test="${page > 1}">
					<c:url var="prevPageUrl" value="/vendor/product.htm">
						<c:if test="${hiddenView}">
							<c:param name="view" value="hidden" />
						</c:if>
						<c:param name="page" value="${page - 1}" />
					</c:url>
					<a class="page-arrow" href="${prevPageUrl}" aria-label="이전 페이지">
						<svg class="icon">
							<use href="#ic-chevron-left" /></svg>
					</a>
				</c:if>

				<c:forEach var="p" begin="1" end="${totalPages}">
					<c:url var="pageUrl" value="/vendor/product.htm">
						<c:if test="${hiddenView}">
							<c:param name="view" value="hidden" />
						</c:if>
				...
				<c:if test="${page < totalPages}">
					<c:url var="nextPageUrl" value="/vendor/product.htm">
						<c:if test="${hiddenView}">
							<c:param name="view" value="hidden" />
						</c:if>
						<c:param name="page" value="${page + 1}" />
